...
function averageWordLength(wordsArrArray) { 
  if (wordsArrArray.length === 0) {
    return null;
  }

  let sum = 0;

  for (let word of wordsArrArray) {
    sum += word.length;
  }

  return sum / wordsArrArray.length;
}

// Bonus - Iteration #4.1
function avg(wordsArrArray) {
  if (wordsArrArray.length === 0) {
    return null;
  }
